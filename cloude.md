...
- [x] NS `lavandabel.by` → ns1/ns2.hoster.by, A → 87.232.64.134 — работает.
      **`.site` / `.online` / `.xyz` НЕ делегированы** (NS-записей нет вообще) —
      проверить делегирование в кабинете hoster.by, потом настроить 301
...
- [ ] Настроить 301-редиректы с `.site` / `.online` / `.xyz` (ждут делегирования)
