...
### Инфраструктура
- [ ] Сменить пароль панели ISPmanager (если ещё не сменён!)
- [x] NS `lavandabel.by` → ns1/ns2.hoster.by, A → 87.232.64.134 — работает
- [x] Все 4 домена активны и оформлены на КФХ «Лавандовая ферма» (проверено
      в кабинете cp.hoster.by 02.09.2026) — админ домена юрлицо, вопрос закрыт
- [x] NS для `.site` / `.online` / `.xyz` изменены на ns1/ns2.hoster.by
      через cp.hoster.by → «Сменить NS-записи»
- [x] Сайт `lavandabel.by` (+псевдоним www) создан, PHP LSAPI 8.2
- [x] Сборка залита 02.09.2026 (через Менеджер файлов, zip + распаковка),
      `.htaccess` работает (SPA-fallback проверен)
- [x] Let's Encrypt выпущен (`lavandabel.by_le2`, включая www);
      включены редиректы: HTTP→HTTPS и www→без www (проверены, 301)
- [x] `config.php` с токеном бота лежит выше корня сайта (`www/config.php`),
      по HTTP недоступен (проверено); форма `send.php` работает —
      тестовая заявка дошла до Telegram
- [ ] **Создать сайты для `.site` / `.online` / `.xyz` в ISPmanager + 301-редиректы.**
      Диагностика 04.09.2026: реестры зон уже указывают на ns1/ns2.hoster.by,
      но на самих ns зон для этих доменов НЕТ (`dig @ns1.hoster.by lavandabel.site
      SOA` → SERVFAIL, при этом `lavandabel.by` → NOERROR). Делегирование
      разорвано — «подождать пропагацию» не поможет. Зона создаётся
      автоматически при создании сайта в ISPmanager. Порядок для каждого домена:
      1. «Сайты» → «Создать сайт», имя `lavandabel.site`, псевдоним
         `www.lavandabel.site`, корень оставить по умолчанию
      2. Дождаться DNS (15–30 мин), выпустить Let's Encrypt
      3. В корень сайта положить `.htaccess` с 301 на основной домен:
         ```apache
         <IfModule mod_rewrite.c>
           RewriteEngine On
           RewriteRule ^(.*)$ https://lavandabel.by/$1 [R=301,L]
         </IfModule>
         ```
      Лимит тарифа — 5 сайтов, занят 1, хватает
- [ ] Убрать домен из проекта Vercel, деплой удалить — можно уже сейчас:
      домен смотрит на hoster.by, контент и картинки от Vercel не зависят
- [ ] Подать заявку на регистрацию в БелГИЭ через кабинет hoster.by
- [ ] SSH: порты 22/21 закрыты — написать в поддержку hoster.by, чтобы включили
      SSH для h216118 (ключ `lavanda-deploy` уже в `~/.ssh/authorized_keys`).
      После этого заработают rsync-деплой и GitHub Action
- [ ] Добавить в репо workflow'ы: деплой по пушу (ждёт SSH), Supabase
      keepalive и бэкап (сниппеты в этом файле, секреты в GitHub)
